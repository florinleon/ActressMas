    ...
    public class EnvironmentMas
    {
        ...
        internal List<ObservableAgent> GetListOfObservableAgents(string perceivingAgentName, Func<Dictionary<string, string>, bool> PerceptionFilter)
        {
            lock (_locker)
            {
                ...
                {
                    Agent a;
                    if (!Agents.TryGetValue(agentsNames[i], out a))
                        continue;
        ...
        private void RunTurn(int turn)
        {
            if (!_parallel)
            {
                ...
                int count = agentNames.Length;

                var agentOrder = _randomOrder ? RandomPermutation(count) : SortedPermutation(count);

                for (int i = 0; i < count; i++)
                {
                    string name = agentNames[agentOrder[i]];

                    if (!Agents.ContainsKey(name))  // agent may have been stopped/removed
                        continue;

                    var agent = Agents[name];

                    if (agent.MustRunSetup)  // first turn runs Setup
                        ExecuteSetup(agent);
                    else
                        ExecuteSeeAct(agent);
                }
            ...
            {
                var agents = Agents.Values.ToArray();
                var actions = new Action[agents.Length];

                for (int i = 0; i < agents.Length; i++)
                {
                    Agent agent = agents[i];

                    if (agent.MustRunSetup)
                        actions[i] = () => ExecuteSetup(agent);
                    else
                        actions[i] = () => ExecuteSeeAct(agent);
                }
