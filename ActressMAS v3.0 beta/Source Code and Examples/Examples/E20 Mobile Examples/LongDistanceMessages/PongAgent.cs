...
namespace LongDistanceMessages
{
    public class PongAgent : Agent
    {
        ...
        {
            Global.Rtb.AppendText($"[{this.Name}] Waiting for requests...\r\n");
        }
        ...
        public override void Act(Message message)
        {
            try
            {
                ...
                {
                    string info = $"Info 1 from agent {this.Name} in container {this.Environment.ContainerName}";
                    Send(message.Sender, info);

                    info = $"Info 2 from agent {this.Name} in container {this.Environment.ContainerName}";
                    Send(message.Sender, info);
                }
            }
            catch (Exception ex)
