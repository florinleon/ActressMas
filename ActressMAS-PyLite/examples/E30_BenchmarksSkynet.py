...
class MyAgent(Agent):
    ...
    def setup(self):
        ...
        if self._messages_left == 0:
            self.send(f"a{self._parent_id}", f"{self._id}")  # send id to parent
            self.stop()


    def act(self, m):
        self._sum += int(m.content)
        self._messages_left -= 1

        if self._messages_left == 0:
            if self.name == "a0":
                print(f"Sum = {self._sum}")  # root reporting
            else:
                self.send(f"a{self._parent_id}", f"{self._sum}")  # send sum to parent
            self.stop()
